...
function PDFUploadCtrl($scope, PDF, AbsenceRecord, Auth, School, toastr) {
  ...
  function createSomethings(entry, prevEntry, school, schoolYear) {
  ...
  function completeRecord(partialRecord) {
    ...
    _.forEach(record.updates || [], function(student) {
      ...
      entry.somethings =
        createSomethings(entry, prevEntry, school, partialRecord.schoolYear);
    });
    _.forEach(record.creates || [], function(student) {
      ...
      entry.somethings =
        createSomethings(entry, {}, school, partialRecord.schoolYear);
    });
